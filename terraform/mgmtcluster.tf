...
provisioner "file" {
  ...
  provisioner "file" {
    content     = templatefile("files/template/clusterctl.yaml.tmpl", { kubernetes_version = var.kubernetes_version, availability_zone = var.availability_zone, external = var.external, image = var.image, flavor = var.flavor, cloud_provider = var.cloud_provider, dns_domain = var.dns_domain, clouds = local.clouds })
  ...
  provisioner "file" {
    content     = templatefile("files/template/clusterctl_template.sh.tmpl", { CLOUD_PROVIDER = var.cloud_provider })
  ...
  provisioner "file" {
    source      = "files/attach_seed.sh"
    destination = "/home/${var.ssh_username}/attach_seed.sh"
  }
